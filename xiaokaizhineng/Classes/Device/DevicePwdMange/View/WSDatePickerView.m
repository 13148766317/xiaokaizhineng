...
typedef void(^startBlock)(NSDate *);
typedef void(^endBlock)(NSDate *);
...
/**
 默认滚动到当前时间
 */
-(instancetype)initWithDateStyle:(WSDateStyle)datePickerStyle startOrEnd:(NSString*)startOrEnd StartBlock:(void(^)(NSDate *))startBlock EndBlock:(void(^)(NSDate *))endBlock {
    self = [super init];
    if (self) {
        ...
        if ([startOrEnd isEqualToString:@"start"]) {
            self.startBlock = ^(NSDate *selectDate) {
                startBlock(selectDate);
            };
        }else if ([startOrEnd isEqualToString:@"end"]){
            self.endBlock = ^(NSDate *selectDate) {
                endBlock(selectDate);
            };
        }
    }
...
/**
 滚动到指定的的日期
 */
-(instancetype)initWithDateStyle:(WSDateStyle)datePickerStyle scrollToDate:(NSDate *)scrollToDate startOrEnd:(NSString*)startOrEnd StartBlock:(void(^)(NSDate *))startBlock EndBlock:(void(^)(NSDate *))endBlock {
    self = [super init];
    if (self) {
        self = [[[NSBundle mainBundle] loadNibNamed:NSStringFromClass([self class]) owner:self options:nil] lastObject];
        self.datePickerStyle = datePickerStyle;
...
-(void)defaultConfig {
    ...
    if (!self.minLimitDate) {
        self.minLimitDate = [NSDate dateWithTimeIntervalSinceNow:0];
    }
}
...
-(void)addLabelWithName:(NSArray *)nameArr {
    ...
    for (int i=0; i<nameArr.count; i++) {
        ...
        UILabel *label = [[UILabel alloc]initWithFrame:CGRectMake(labelX, self.showYearView.frame.size.height/2+3, 15, 15)];
...
- (IBAction)doneAction:(UIButton *)btn {
    ...
    if ([self.startOrEnd isEqualToString:@"start"]) {
        self.startBlock(_startDate);
    }else if ([self.startOrEnd isEqualToString:@"end"]){
        self.endBlock(_startDate);
    }
    [self dismiss];
}
...
-(UIPickerView *)datePicker {
    if (!_datePicker) {
        ...
        _datePicker = [[UIPickerView alloc] initWithFrame:CGRectMake(0, 10,_showYearView.frame.size.width-20, _showYearView.frame.size.height)];
