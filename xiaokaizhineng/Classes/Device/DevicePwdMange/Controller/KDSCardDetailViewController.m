...
@interface KDSCardDetailViewController ()
...
@implementation KDSCardDetailViewController
...
-(void)navRightClick{
    ...
    UIAlertController *ac = [UIAlertController alertControllerWithTitle:Localized(@"tips") message:Localized(@"ensureDeleteCard?") preferredStyle:UIAlertControllerStyleAlert];
    ...
    UIAlertAction *okAction = [UIAlertAction actionWithTitle:Localized(@"ok") style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        ...
        [weakSelf.lock.bleTool manageKeyWithPwd:@"" userId:weakSelf.model.num action:KDSBleKeyManageActionDelete keyType:KDSBleKeyTypeRFID completion:^(KDSBleError error) {
            if (error == KDSBleErrorSuccess)
            {
                [hud hide:NO];
                [MBProgressHUD showSuccess:Localized(@"deleteSuccess")];
                !weakSelf.cardHasBeenDeletedBlock ?: weakSelf.cardHasBeenDeletedBlock(weakSelf.model);
...
///点击编辑按钮修改卡片用户昵称。
- (IBAction)clickEditBtnAlterCardNickname:(UIButton *)sender
{
       __weak typeof(self) weakSelf = self;
    ...
    UIAlertAction *okAction = [UIAlertAction actionWithTitle:Localized(@"ok") style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        ...
        [[KDSHttpManager sharedManager] setBlePwd:weakSelf.model withUid:[KDSUserManager sharedManager].user.uid bleName:weakSelf.lock.device.device_name success:^{
            ...
            !weakSelf.cardInfoDidUpdateBlock ?: weakSelf.cardInfoDidUpdateBlock(weakSelf.model);
   ...
   ///门卡名称文本输入框，限制文本长度小于16，大于16取值到16且提示
